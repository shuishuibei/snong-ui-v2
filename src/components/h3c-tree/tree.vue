 ...
 * @LastEditors: ys2234
 * @LastEditTime: 2019-09-20 11:55:42
 -->
<template>
  <div :class="prefixCls">
    <tree-node
...
export default {
  ...
  watch: {
    data: {
      ...
      handler() {
        ...
        this.addSearchText(this.stateTree, this.searchText);
  ...
  methods: {
    /**
     * @function 添加搜索
     * @param {Array} arr - 需要被查询的数据内容
     * @param {string} str - 需要查询的数据
     */
    addSearchText(arr, str) {
      for (let i = 0, len = arr.length; i < len; i++) {
        this.$set(arr[i], 'searchText', str);
        if (arr[i].children && arr[i].children.length) {
          this.addSearchText(arr[i].children, str);
        }
      }
    },
    compileFlatState() {
