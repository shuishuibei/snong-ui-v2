<template>
    <div class="cds-wrapper cds-border cds-padding-14">
        ...
        <div >
            ...
            <div class="cds-clearfix cds-margin-bottom-14">
              ...
              <i-button
                type="primary"
                @click="handleExport">
                <i class="iconfont icon-xiazai">数据统计</i>
              </i-button>
            <!-- 按钮操作 -->
        ...
        <!-- 新增 -->
        <TaskManageEdit v-model="flag.add"
                        title="新增技术任务"
                        :dataList="{add: true}">
        </TaskManageEdit>
        <!-- 修改 -->
        <TaskManageEdit v-model="flag.update"
                        title="修改技术任务"
                        :dataList="currentRow">
        </TaskManageEdit>
        <!-- 单条删除 -->
        <ModalConfirm ref="deleteSingle"
                      ...
                      title="删除技术任务"
                      :info="'确认删除：【' + currentRow.taskTitle + '】技术任务吗？'"
                      @on-cancel="deleteIds = []"
                      @on-ok="submitDelete"
                      :showBackgroudColor="false">
        </ModalConfirm>
      <!-- 发送邮件 -->
      <ModalConfirm ref="noticeMail"
                    v-model="flag.notice"
                    title="发送通知邮件"
                    :info="'确认发送：【' + currentRow.taskTitle + '】技术任务的通知邮件吗？'"
                    @on-ok="submitNotice"
                    :showBackgroudColor="false">
      </ModalConfirm>
      <!-- 下载附件 -->
      <ModalConfirm ref="downloadFile"
                    ...
                    title="下载技术任务附件"
                    :info="'确认下载：【' + currentRow.taskTitle + '】技术任务排查的所有附件吗？'"
...
const TASK_LEVEL = [
  ...
  },
  {
    ...
    import TaskManageEdit from './components/TaskManageEdit.vue';
    import H3cModalDetail from '@/components/h3c-modal-detail/H3cModalDetail.vue';

    export default {
        name: "task-manage",
        components: {ModalConfirm, TaskManageEdit, H3cModalDetail},
        data() {
            return {
                ...
                tableConfig: {
                    ...
                    columns: [
                      ...
                      {
                        title: '技术任务标题',       // 列头显示文字
                        key: 'taskTitle',    // 对应列内容的字段名，需要与表格绑定的data中对象的属性匹配
                        ellipsis: true,
                          ...
                          return h('span', {
                            ...
                            on: {
                              click: () => {
                                ...
                                this.$http.get(`task/${this.currentRow.id}`).then(({data}) => {
                                  if (data.status) {
                                    this.detailData = {...this.currentRow}
                                    this.detailData.unhandledNames = data.data.unhandledNames;
                                    this.detailData.notRelatedNames = data.data.notRelatedNames;
                                    this.detailData.processingNames = data.data.processingNames;
                                    this.detailData.completedNames = data.data.completedNames;
                                  }
                                })
                                this.flag.info = true;
                              }
                            }
                          }, params.row.taskTitle);
                        }
                      },
                      {
                        title: '技术任务内容',
                        ...
                        render: (h, params) => {
                          return h('span', {
                            attrs: {
                              title: params.row.taskContent
                            }
                          }, params.row.taskContent);
                        }
                      },
                      {
                        title: '技术公告级别',
                        ...
                        ellipsis: true,
                        render: (h, params) => {
                          let level = params.row.taskLevel,
                            node = TASK_LEVEL.filter((item) => {
                      ...
                      },
                      {
                        ...
                        render: (h, params) => {
                          return h('span', {
                            attrs: {
                              title: params.row.unhandledCount
                            },
                            style: {
                              color: "red"
                            }
                          }, params.row.unhandledCount);
                        }
                      },
                      {
                        title: '不涉及数量',
                        ...
                        render: (h, params) => {
                          return h('span', {
                            attrs: {
                              title: params.row.notRelatedCount
                            }
                          }, params.row.notRelatedCount);
                        }
                      },
                      {
                        title: '处理中数量',
                        ...
                        render: (h, params) => {
                          return h('span', {
                            attrs: {
                              title: params.row.processingCount
                            }
                          }, params.row.processingCount);
                        }
                      },
                      {
                        title: '已完成数量',
                        ...
                        render: (h, params) => {
                          return h('span', {
                            attrs: {
                              title: params.row.completedCount
                            },
                            style: {
                              color: "green"
                            }
                          }, params.row.completedCount);
                        }
                      },
                        {
                            title: '创建时间',
        ...
        methods: {
          ...
          handleExport() {
            ...
            this.$http.get(
              ...
              .then((res)=>{
                ...
                a.href = url;
                a.download = "技术公告任务" + this.$moment().format('YYYYMMDD') + ".xlsx"
                a.click();
            ...
            //提交删除
            submitDelete() {
                this.$http.delete(`task/${this.currentRow.id}/delete`).then(({data}) => {
          ...
          submitDownload() {
            this.$http({
              method: 'GET',
              url: `/task/download/${this.currentRow.id}?type=zip`,
              responseType: 'blob'
            }).then((res)=>{
              let a = document.createElement('a'),
                  url = window.URL.createObjectURL(res.data),
                  filename = this.currentRow.taskTitle;
              a.href = url;
              a.download = filename + "-" + this.$moment().format('YYYYMMDD') + ".zip"
              a.click();
              window.URL.revokeObjectURL(url);
          ...
          //提交下载
          submitNotice() {
            this.$http.get(`task/notice/${this.currentRow.id}`).then(({data}) => {
              //删除成功后需要重新获取数据，表格显示缓冲，定位到第一页（操作失败不作处理，已在入口文件做了统一处理）
              if (data.status) {
                //删除成功后的提示信息
                this.$Notice.success({
