...
export default {
            ...
            render: (h, params) => {
              return h('div', [
                  h('i', {
                    class: {
                      'iconfont': true,
                      'icon-shipinhuifang': true
                    },
                    on: {
                      click: () => {
                          // 设置当前操作行
                          let row = params.row;
                          let url = `host/playback/${row.videoId}`;
                          window.open(url);
                      }
                    }
                  })
  ...
  methods: {
    ...
    handleSearch() {
      ...
      this.searchMap.startTime = this.pickStartDate.trim()
      this.searchMap.endTime = this.pickEndDate.trim()
    ...
    setInputEmpty() {
      ...
      this.searchMap.startTime = this.pickStartDate = '';
      this.searchMap.endTime = this.pickEndDate = '';
    },
    //表格选中项发生变化时触发
    ...
    getTableData({loading = false, currentPage = 1, pageSize = 10} = {}) {
      ...
      this.deleteIds = [];
      //发送请求并且做相应处理
      this.$http.post(`host/audit/page/list`, {
        current: currentPage,
