 ...
 * @LastEditTime 2019-11-06 15:32:13
 -->
<template>
...
export default {
  ...
  data() {
    return {
      config: {
        ...
        columns: [
          ...
          },
          {
            ...
            ellipsis: true,
            render: (h, params) => {
              let text = params.row.processStartTime;
              return h(
                'div',
                {
                  attrs: { title: text }
          ...
          },
          {
            ...
            ellipsis: true,
            render: (h, params) => {
              let text = params.row.taskCreateTime;
              return h(
                'div',
                {
                  attrs: { title: text }
      ...
      httpUrl: {
        urlList: 'act-manage/todo-task/list/page' // 表格列表的请求地址
      }
    };
  },
  beforeRouteEnter(to, from, next) {
    if (to.params.isRefresh) {
      next(vm => {
        vm.getTableData();
      });
    } else {
