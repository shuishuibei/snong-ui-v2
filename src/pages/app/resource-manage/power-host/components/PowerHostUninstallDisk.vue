<!--
 * @Description: power虚拟机卸载硬盘弹窗
 * @Author: lys1626
 * @Date: 2019-09-06 13:38:06
 * @LastEditors: lys1626
 * @LastEditTime: 2019-09-11 17:21:21
 -->
<template>
  <i-modal
    v-model="visible"
    ...
    <div class="cds-clearfix cds-margin-bottom-14">
      ...
      <div class="cds-float-right-btn" v-show="isBatchDeletion">
        <i-button type="primary" @click="handleUninstallDiskBatch">
          <i class="iconfont iconfont icon-xiezai"></i>
          批量卸载
        </i-button>
      </div>
    </div>
      ...
      <i-row class="cds-clearfix cds-margin-top-14">
        ...
        <div class="cds-float-right">
          <i-page
            ...
            @on-change="submitUnloadDisk({currentPage: $event, pageSize: page.pageSize})"
            @on-page-size-change="submitUnloadDisk({pageSize: $event})"
            placement="top"
...
<script>
import { powerDisk } from '@/assets/js/status/index.js';
import H3cTitle from '@/components/h3c-title/H3cTitle.vue'; // 引入全局组件title组件

export default {
  name: 'PowerHostUninstallDisk',
  components: {
    'h3c-title': H3cTitle
  },
  props: {
  ...
  methods: {
    ...
    filterStatusRemote(filters, key) {
      ...
      if (this.hostFilter.status == 'in_use') {
        this.hostFilter.status = 'IN_USE';
      } else if (this.hostFilter.status == 'available') {
        this.hostFilter.status = 'AVAILABLE';
      }
      this.clearChoosed();
      this.submitUnloadDisk();
    },
     ...
     * @param {Object} 分页默认数据
     */
    submitUnloadDisk({ loading = false, currentPage = 1, pageSize = 5 } = {}) {
      if (loading) {
            ...
            if (data.status) {
              ...
              if (
                ...
                if (this.tableData.length > 1) {
                  this.isBatchDeletion = true; // 数据多于一条时，显示批量挂载按钮与复选框，进行批量挂载
     ...
     * @description: 批量卸载硬盘
     */
    handleUninstallDiskBatch() {
      this.isSingleUninstall = false;
    ...
    handleUninstallDisk() {
      ...
      if (this.serverIds.length == 0 || this.hostId == '') {
        this.$Notice.warning({
          desc: '至少选择一项要卸载的硬盘！'
        });
      } else {
