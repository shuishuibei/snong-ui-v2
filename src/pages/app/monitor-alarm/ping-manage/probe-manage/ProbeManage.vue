<template>
    <div class="cds-wrapper cds-border cds-padding-14">
      ...
      <!-- 新增 -->
      <ProbeEdit v-model="flag.add"
                      title="新增探针节点"
                      :dataList="{add: true}">
      </ProbeEdit>
      <!-- 修改 -->
      <ProbeEdit v-model="flag.update"
                      title="修改探针节点"
                      :dataList="currentRow">
      </ProbeEdit>
      <!-- 单条删除 -->
      <ModalConfirm ref="deleteSingle"
                    v-model="flag.delete"
                    title="删除探针节点"
                    :info="'确认删除：【' + currentRow.pingNodeName + '[' + currentRow.pingIp +']' + '】探针节点吗？'"
                    @on-cancel="deleteIds = []"
                    @on-ok="submitDelete"
      ...
      <ModalConfirm ref="deleteBatch"
                    ...
                    title="删除探针节点"
                    :info="`确认删除这${deleteIds.length}个探针节点？`"
                    @on-cancel="onCancel"
                    @on-ok="submitDelete"
                    :showBackgroudColor="false">
      </ModalConfirm>
      ...
      <ModalConfirm ref="deleteSingle"
                    v-model="flag.deploy"
                    title="部署节点探针"
                    :info="'确认部署【' + currentRow.pingNodeName + '[' + currentRow.pingIp +']' + '】探针吗？'"
                    @on-cancel="deleteIds = []"
                    @on-ok="submitDeploy"
                    :showBackgroudColor="false">
      </ModalConfirm>
...
<script>
import ProbeEdit from './components/ProbeEdit';
...
import { DetailRow } from './components/DetailColumn';
import * as API from './components/api';
export default {
  name: "probe-manage",
  components: {ModalConfirm, H3cModalDetail, ProbeEdit},
    data() {
        return {
            /**
            ...
            tableConfig: {
              ...
              columns: [
                ...
                {
                  title: '探针名称',       // 列头显示文字
                  key: 'pingNodeName',    // 对应列内容的字段名，需要与表格绑定的data中对象的属性匹配
                  ellipsis: true,
                  minWidth: 150,
                ...
                },
                {
                  ...
                  render: (h, params) => {
                    let region = params.row.region,
                      node = API.NETWORK_AREA.filter((item) => {
                ...
                },
                {
                  ...
                  render: (h, params) => {
                    let pingObject = params.row.pingObject,
                      node = API.PING_OBJECTS.filter((item) => {
                ...
                },
                {
                  ...
                  render: (h, params) => {
                    return h('span', {
                      attrs: {
                        title: params.row.pingPublicIp
                      }
                    }, params.row.pingPublicIp);
                  }
                },{
                  title: '部署状态',
                  key: 'probeStatus',
    ...
    methods: {
        ...
        handleSearch() {
          ...
          this.searchMap.pingNodeName = this.inputProbeName.trim()
          this.searchMap.pingIp = this.inputIp.trim()
        ...
        setInputEmpty() {
          ...
          this.searchMap.pingNodeName = this.inputProbeName = '';
          this.searchMap.pingIp = this.inputIp = '';
        },

      //提交部署
      submitDeploy() {
        this.$loading.show();
        this.$http.get(`probeNode/${this.currentRow.id}/deploy`).then(({data}) => {
          //删除成功后需要重新获取数据，表格显示缓冲，定位到第一页（操作失败不作处理，已在入口文件做了统一处理）
          if (data.status) {
            this.$Notice.success({
              desc: data.message && data.message !== '' ? data.message : '部署节点探针成功。'
            });
            this.getTableData({loading: true});
          } else {
            this.$Notice.error({
              desc: data.message && data.message !== '' ? data.message : '部署节点探针失败。'
            });
          }
          this.$loading.hide();
        })
