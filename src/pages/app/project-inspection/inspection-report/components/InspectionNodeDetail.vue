     ...
     getColumns(){
       let columns = [
...

         {
           ...
           render: (h, { row: { nodeType } }) => {
             let tmp = AuthType.filter((item) => item.value === nodeType), text = '', color = '';
             if (tmp.length > 0) {
               text = tmp[0].label
               color = tmp[0].color
             }
    ...
    methods: {
      ...
      handlerChangePage({ current = 1, pageSize = 10 } = {}) {
        ...
        getTableData({loading = false, currentPage = 1, pageSize = 5} = {}) {
            ...
            //发送请求并且做相应处理
            this.$http.post(`inspection/node/page/list`, {
                current: currentPage,
                orderMap: this.orderMap,
                pageSize: pageSize,
                searchMap: {
                  ip: this.searchValue,
                  inspectionDate: this.data.dateTime,
