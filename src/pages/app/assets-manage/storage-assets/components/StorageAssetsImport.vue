...
export default {
  ...
  methods: {
    ...
    download() {
      this.$http({
        method: 'GET',
        url: 'assets/storage/download',
        responseType: 'blob'
      }).then((res)=>{
        let a = document.createElement('a'),
          url = window.URL.createObjectURL(res.data);
        a.href = url;
        a.download = 'assets_storage_template.xlsx';
        a.click();
    ...
    ok() {
      ...
      this.$refs['form'].validate((valid) => {
        ...
        if (valid) {
          ...
          this.$http.post('assets/storage/import', formData)
            .then(({data}) => {
              ...
              if (data.status) {
                ...
                // 新增成功后的提示信息
                this.$Notice.error({
                  desc: data.message && data.message !== '' ? data.message : '导入存储设备资产数据失败。'
                });
              }
            })
        }
