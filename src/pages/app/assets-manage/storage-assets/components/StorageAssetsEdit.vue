  ...
  <Modal v-model="visible"
    ...
    <Form ref="form" :model="formObject" :rules="ruleValidate" :label-width="100">
      ...
      <FormItem prop="deviceModel" label="设备类型">
        <i-input type="text"
                 v-model="formObject.deviceModel"></i-input>
      </FormItem>
      <FormItem prop="deviceType" label="设备型号">
        <i-input type="text"
                 v-model="formObject.deviceType"></i-input>
      </FormItem>
      <FormItem prop="ilo" label="ILO">
        <i-input type="text"
                 v-model="formObject.ilo"></i-input>
      </FormItem>
  ...
  methods: {
    ...
    ok() {
      ...
      this.$refs['form'].validate((valid) => {
        ...
        if (valid) {
          if(this.dataList.add){
            this.$http.post('assets/storage/add', this.formObject)
              .then(({data}) => {
            ...
            this.$http.put('assets/storage/update', this.formObject)
              .then(({data}) => {
                // 新增成功后重新请求表格数据，表格缓冲且保留原每页条数（操作失败不作处理，已在入口文件做了统一处理）
                if (data.status) {
                  // 新增成功后的提示信息
                  ...
                  this.$Notice.error({
                    desc: data.message && data.message !== '' ? data.message : '修改存储设备资产失败。'
                  });
                }
              })
          }
