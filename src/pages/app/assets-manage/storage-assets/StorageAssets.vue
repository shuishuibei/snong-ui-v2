<template>
  <div class="cds-wrapper cds-border cds-padding-14">
    ...
    <!-- 认证 -->
    <StorageAssetsAuth v-model="flag.auth"
                       :title="title"
                       :ids="deleteIds"
                       :dataList="currentRow">
    </StorageAssetsAuth>
    <!-- 新增 -->
    <StorageAssetsEdit v-model="flag.add"
                       title="新增存储设备"
                       :dataList="{add: true}">
    </StorageAssetsEdit>
    <!-- 修改 -->
    <StorageAssetsEdit v-model="flag.update"
                       title="修改存储设备"
                       :dataList="currentRow">
    </StorageAssetsEdit>
    <!-- 导入Excel数据 -->
    <StorageAssetsImport v-model="flag.import"
                         title="导入存储设备"
                         :dataList="currentRow">
    </StorageAssetsImport>
    <!-- 单条删除 -->
    <ModalConfirm ref="deleteSingle"
                  v-model="flag.delete"
                  title="删除存储设备"
                  :info="'确认删除：【' + currentRow.deviceName + '[' + currentRow.ip +']' + '】网络吗？'"
                  @on-cancel="deleteIds = []"
                  @on-ok="submitDelete"
    ...
    <ModalConfirm ref="deleteBatch"
                  ...
                  title="删除存储设备"
                  :info="`确认删除这${deleteIds.length}个存储设备？`"
                  @on-cancel="onCancel"
                  @on-ok="submitDelete"
                  :showBackgroudColor="false">
    </ModalConfirm>
...
import StorageAssetsAuth from './components/StorageAssetsAuth.vue';
import StorageAssetsEdit from './components/StorageAssetsEdit.vue';
import StorageAssetsImport from './components/StorageAssetsImport.vue';
...
export default {
  name: "host-manage",
  components: {ModalConfirm, H3cModalDetail, StorageAssetsEdit,StorageAssetsAuth, StorageAssetsImport},
  data() {
    return {
      /**
        ...
        columns: [
          ...
          },
          {
            ...
            render: (h, params) => {
              return h('span', {
                attrs: {
                  title: params.row.deviceModel
                }
              }, params.row.deviceModel);
            }
          },
          {
            title: '设备型号',
                    ...
                    on: {
                      click: () => {
                        if (active) {
                          ...
                          this.currentRow = params.row;
                          this.onCancel();
                          this.title = '存储设备认证[' + params.row.ip + ']'
                          this.flag.auth = true;
                        }
                      }
                    }
  ...
  methods: {
    ...
    getTableData({loading = false, currentPage = 1, pageSize = 10} = {}) {
      ...
      this.deleteIds = [];
      //发送请求并且做相应处理
      this.$http.post(`assets/storage/page/list`, {
        current: currentPage,
    ...
    //提交删除
    submitDelete() {
      this.$http.post(`assets/storage/delete`,
        this.deleteIds
      ).then(({data}) => {
