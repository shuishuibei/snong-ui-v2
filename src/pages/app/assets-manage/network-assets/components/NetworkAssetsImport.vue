    ...
    export default {
        ...
        methods: {
          ...
          download() {
            this.$http({
              method: 'GET',
              url: 'assets/network/download',
              responseType: 'blob'
            }).then((res)=>{
              let a = document.createElement('a'),
                url = window.URL.createObjectURL(res.data);
              a.href = url;
              a.download = 'assets_network_template.xlsx';
              a.click();
                ...
                this.$refs['form'].validate((valid) => {
                  ...
                  if (valid) {
                    ...
                    formData.append('file',this.formObject.file);
                        this.$http.post('assets/network/import', formData)
                        .then(({data}) => {
                            ...
                            if (data.status) {
                              ...
                              // 新增成功后的提示信息
                              this.$Notice.error({
                                desc: data.message && data.message !== '' ? data.message : '导入网络设备资产数据失败。'
                              });
                            }
                        })
                      }
