            ...
            ok() {
                ...
                this.$refs['form'].validate((valid) => {
                    ...
                    if (valid) {
                      ...
                      this.loading = true;
                        this.$http.post('assets/network/auth', this.formObject).then(({data}) => {
                          // 新增成功后重新请求表格数据，表格缓冲且保留原每页条数（操作失败不作处理，已在入口文件做了统一处理）
                          if (data.status) {
                            // 新增成功后的提示信息
                            this.$Notice.success({
                              desc: data.message && data.message !== '' ? data.message : '设备认证成功。'
                            });
                            ...
                            // 新增成功后的提示信息
                            this.$Notice.error({
                              desc: data.message && data.message !== '' ? data.message : '设备认证失败。'
                            });
                          }
                        })
                      this.loading = false;
