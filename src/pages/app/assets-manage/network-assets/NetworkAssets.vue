<template>
    <div class="cds-wrapper cds-border cds-padding-14">
        ...
        <div >
          ...
          <div class="cds-float-right">
            ...
            <i-button
              ...
              <i class="iconfont icon-zu">导入设备</i>
            </i-button>
            <i-button
              type="primary" :disabled="deleteIds.length==0"
              @click="flag.monitorBatch = true">
              <i class="iconfont icon-renzheng">批量监控</i>
            </i-button>
            <i-button
              type="primary" :disabled="deleteIds.length==0"
      ...
      <!-- 新增 -->
      <NetworkAssetsEdit v-model="flag.add"
                      title="新增网络设备"
                      :dataList="{add: true}">
      </NetworkAssetsEdit>
      <!-- 修改 -->
      <NetworkAssetsEdit v-model="flag.update"
                      title="修改网络设备"
                      :dataList="currentRow">
      </NetworkAssetsEdit>
      <!-- 导入Excel数据 -->
      <NetworkAssetsImport v-model="flag.import"
                      title="导入网络设备"
                      :dataList="currentRow">
      </NetworkAssetsImport>
      <!-- 单条删除 -->
      <ModalConfirm ref="deleteSingle"
                    v-model="flag.delete"
                    title="删除网络资产"
                    :info="'确认删除：【' + currentRow.deviceName + '[' + currentRow.ip +']' + '】网络设备吗？'"
                    @on-cancel="deleteIds = []"
                    @on-ok="submitDelete"
      ...
      <ModalConfirm ref="deleteBatch"
                    ...
                    title="删除网络资产"
                    :info="`确认删除这${deleteIds.length}个网络资产？`"
                    @on-cancel="onCancel"
                    @on-ok="submitDelete"
                    :showBackgroudColor="false">
      </ModalConfirm>
      <!-- 单条监控 -->
      <ModalConfirm ref="deleteSingle"
                    v-model="flag.monitor"
                    title="监控网络资产"
                    :info="'确认监控：【' + currentRow.deviceName + '[' + currentRow.ip +']' + '】网络设备吗？'"
                    @on-cancel="deleteIds = []"
                    @on-ok="submitMonitor"
                    :showBackgroudColor="false">
      </ModalConfirm>
      <ModalConfirm ref="deleteBatch"
                    v-model="flag.monitorBatch"
                    title="监控网络资产"
                    :info="`确认监控这${deleteIds.length}个网络设备吗？`"
                    @on-cancel="onCancel"
                    @on-ok="submitMonitor"
                    :showBackgroudColor="false">
      </ModalConfirm>
...
<script>
import NetworkAssetsAuth from './components/NetworkAssetsAuth.vue';
import NetworkAssetsEdit from './components/NetworkAssetsEdit.vue';
import NetworkAssetsImport from './components/NetworkAssetsImport.vue';
...
const Status = [
  ...
  },
  {
    ...
    export default {
      name: "network-manage",
      components: {ModalConfirm, H3cModalDetail, NetworkAssetsEdit,NetworkAssetsAuth, NetworkAssetsImport},
        data() {
            return {
                /**
                ...
                tableConfig: {
                    ...
                    columns: [
                      ...
                      {
                        title: '设备名称',       // 列头显示文字
                        key: 'deviceName',    // 对应列内容的字段名，需要与表格绑定的data中对象的属性匹配
                        ellipsis: true,
                        minWidth: 120,
                      ...
                      },
                      {
                        ...
                        ellipsis: true,
                        render: (h, params) => {
                          let deviceModel = params.row.deviceModel,
                            node = DEVICE_MODEL_LIST.filter((item) => {
                              return item.value == deviceModel
                            })[0];
                            return h('div',
                              {
                                attrs: {
                      ...
                      },
                      {
                        ...
                        ellipsis: true,
                        render: (h, params) => {
                          let node = params.row.status,
                            color = node === '使用' ? '#55ce5b' : '#e31839';
                          return h('div', [
                            h('span', {
                        ...
                        render: (h, params) => {
                          let node = params.row.monitorStatus,
                            text = node === 1 ? '已监控' : '未监控',
                                ...
                                on: {
                                  click: () => {
                                    if (active) {
                                      ...
                                      this.currentRow = params.row;
                                      this.onCancel();
                                      this.deleteIds = [this.currentRow.id];
                                      this.title = '增加网络设备监控[' + this.currentRow.ip + ']'
                                      this.flag.monitor = true;
                                    }
                                  }
                                }
                              })
              ...
              detailRows: [
                ...
                },
                {
                  ...
                  render: (h, params) => {
                    let deviceModel = params.row.deviceModel;
                    if (deviceModel) {
                      let node = DEVICE_MODEL_LIST.filter((item) => {
                        return item.value == deviceModel
                      })[0];
                      return h('div', [
                        h('span', {
        ...
        methods: {
            ...
            handleSearch() {
              ...
              this.searchMap.deviceName = this.inputDeviceName.trim()
          ...
          onCancel(){
            ...
            setInputEmpty() {
              ...
              this.searchMap.deviceName = this.inputDeviceName = '';
            ...
            getTableData({loading = false, currentPage = 1, pageSize = 10} = {}) {
                ...
                this.deleteIds = [];
                //发送请求并且做相应处理
                this.$http.post(`assets/network/page/list`, {
                    current: currentPage,
          ...
          submitMonitor() {
            this.$loading.show();
            this.$http.post(`assets/network/monitor/add`,
              this.deleteIds
            ).then(({data}) => {
              //删除成功后需要重新获取数据，表格显示缓冲，定位到第一页（操作失败不作处理，已在入口文件做了统一处理）
              if (data.status) {
                this.$Notice.success({
                  desc: data.message && data.message !== '' ? data.message : '增加网络资产监控成功。'
                });
                this.getTableData({loading: true});
              } else {
                this.$Notice.error({
                  desc: data.message && data.message !== '' ? data.message : '增加网络资产监控失败。'
                });
              }
              this.$loading.hide();
            })
          ...
          //提交删除
          submitDelete() {
            this.$http.post(`assets/network/delete`,
              this.deleteIds
            ).then(({data}) => {
              //删除成功后需要重新获取数据，表格显示缓冲，定位到第一页（操作失败不作处理，已在入口文件做了统一处理）
              if (data.status) {
