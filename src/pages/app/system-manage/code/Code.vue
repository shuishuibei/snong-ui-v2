<template>
  <div class="cds-wrapper cds-border cds-padding-14">
    ...
    <ModalConfirm ref="deleteSingle"
                  ...
                  title="删除表"
                  :info="'确认删除表：' + currentRow.tableName + '吗？'"
                  @on-cancel="deleteIds = []"
                  @on-ok="submitDelete"
...
export default {
  name: "code",
  components: {ModalConfirm, ModalDetail},
  data() {
    return {
      ...
      tableConfig: {
        ...
        columns: [
          ...
          {
            title: '表名称',       // 列头显示文字
            key: 'tableName',    // 对应列内容的字段名，需要与表格绑定的data中对象的属性匹配
            width: 200,
            ellipsis: true,
  ...
  methods: {
    ...
    handleSearch() {
      ...
      this.searchMap.titleName = this.inputValue.trim();
    ...
    submitSync(){
      this.$loading.show();
      this.$http.get(`tool/gen/db/sync`).then(({data}) => {
        ...
        if (data.status) {
          ...
          this.$Notice.error({
            desc: data.message && data.message !== '' ? data.message : '同步数据库表数据失败。'
          });
        }
        this.$loading.hide();
      })
    ...
    getTableData({loading = false, currentPage = 1, pageSize = 10} = {}) {
      ...
      this.deleteIds = [];
      //发送请求并且做相应处理
      this.$http.post(`tool/gen/page/list`, {
        current: currentPage,
