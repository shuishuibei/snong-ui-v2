 ...
 * @LastEditors: ys2234
 * @LastEditTime: 2019-11-18 10:15:07
 -->
<template>
  <div class="resource-pool-edit cds-padding-14 cds-border cds-scroll-wrap">
      ...
      <i-form
        ...
        <h3c-mall
          ...
          <template slot="mallContent">
            <i-form-item label="资源池名称" prop="name">
        ...
        <h3c-mall class-name="icon-yumingguanli" title="可用域" style-set="#ffa904">
          <template slot="mallContent">
            <i-form-item prop="quotaParam['azoneList']" :label-width="40">
              <i-checkbox-group class="azone-box" v-model="groupDetail.quotaParam['azoneList']">
                <i-row align="middle">
                  <i-col :xs="24" :md="12" :lg="4" v-for="item in azoneList" :key="item.id">
        ...
        <div v-for="(item, index) in quotaConfig" :key="index">
          <h3c-mall :class-name="item.icon" :title="item.label" :style-set="item.color">
            <template slot="mallContent">
              <i-row>
                <i-col :xs="24" :lg="8" v-for="type in item.children" :key="type.value">
...
/**
 * @property {} id                                -
 * @property {string} name                        - 资源池名称
 * @property {} label                             -
 * @property {string} description                 - 备注
 * @property {} parentId                          - 当前配额组织ID
 * @property {} parentName                        -
 * @property {} active                            -
 * @property {} principal                         -
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        name: [
          required({ message: '资源池名称不能为空' }),
          sameName({ message: '资源池名称格式不正确' })
        ],
        telephone: [allPhone()]
      },
      azoneList: [], // 可用域列表
  ...
  methods: {
    ...
    changeCheckBox(value, type) {
      this.groupDetail.quotaParam[type] = value
        ? this.quotaMax[type]
        : this.quotaAssigned[type];
    },
    /**
     * @function 隐藏配额范围提示
      ...
      this.$refs['groupForm'].validate(valid => {
        if (valid) {
          ...
          let obj = {},
            url = '',
            message = '';
          obj.sysPoolDto = { ...this.groupDetail };
          obj.quotaParam = this.groupDetail.quotaParam;
          delete obj.sysPoolDto.quotaParam;
          if (this.$route.meta['breadcrumb'] === '新增资源池') {
            url = 'quota/pool/add';
            message = '新增资源池成功';
          } else {
            url = 'quota/pool/update';
            message = '修改资源池成功';
          }
          this.$http({
            method: 'POST',
    ...
    setRange() {
      for (let type in quotaDefault) {
        ...
        this.quotaRange[
          type
        ] = `有效范围为${this.quotaMin[type]}-${this.quotaMax[type]}`;
      }
    },
    /**
     * @function 获取可用域列表
      ...
      return this.$http.get('quota/root').then(({ data }) => {
        if (data.status) {
          ...
          if (quota) {
            ...
            this.quotaMax.instances =
              quota.maxTotalInstances - quota.totalInstancesUsed; // 主机数量
            ...
            this.quotaMax.snapshots =
              quota.maxTotalSnapshots - quota.totalSnapshotsUsed; // 快照数量
            this.quotaMax.volumes =
              quota.maxTotalVolumes - quota.totalVolumesUsed; // 硬盘数量
            this.quotaMax.router = quota.maxRouters - quota.totalRoutersUsed; // 路由器数量
            this.quotaMax.firewall =
              quota.maxFirewalls - quota.totalFirewallsUsed; // 防火墙数量
            this.quotaMax.network = quota.maxNetworks - quota.totalNetworksUsed; // 网络数量
            this.quotaMax.loadbalancer =
              quota.maxLoadbalancers - quota.totalLoadbalancersUsed; // 负载均衡数量
            this.quotaMax.vnic = quota.maxVnic - quota.totalVnicUsed; // 网卡数量
            this.quotaMax.floatingIp =
              quota.maxTotalFloatingIps - quota.totalFloatingIpsUsed; // 公网IP数量
            ...
            this.parentId = data.data.quota.groupId;
          }
        }
      });
    },
     ...
     * @param {string} poolId  - 当前资源池ID
     */
    getPoolById(poolId) {
      return this.$http.get(`/quota/group/pool/${poolId}`).then(({ data }) => {
        if (data.status) {
          let quota = data.data.quota;
          /* 配额可用最大值 */
  ...
  created() {
    ...
    if (this.$route.query.id) {
      this.title = '修改资源池';
      this.$http
        .get(`quota/pool/${this.$route.query.id}`)
        .then(({ data }) => {
          if (data.status) {
            this.node = data.data;
            let azone2 = this.getAzone(this.node.id);
            let quota2 = this.getPoolById(this.node.id);
            Promise.all([azone2, quota2])
              .then(() => {
                this.groupDetail = { ...this.groupDetail, ...this.node }; // 设置默认信息
                this.groupDetail.telephone = this.node.telephone
                  ? this.node.telephone
                  : '';
      ...
      Promise.all([azone1, quota1])
        .then(() => {
          this.groupDetail.parentId = this.parentId;
          this.groupDetail.quotaParam = {
            ...this.groupDetail.quotaParam,
            ...this.quotaAssigned
          };
  ...
  beforeRouteEnter(to, from, next) {
    /* id存在，修改 */
    if (to.query.id) {
      to.meta.breadcrumb = '修改资源池';
    } else {
      to.meta.breadcrumb = '新增资源池';
    }
    next();
  },
  beforeRouteLeave(to, from, next) {
    if (
      this.operation === 'cancel' &&
      to.name === 'resource-pool-manage/index'
    ) {
