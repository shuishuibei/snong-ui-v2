 ...
 * @LastEditTime: 2019-09-18 11:39:03
 -->
<template>
...
<script>
   ...
   * 详情见：http://vue-charts.npmbaidu.com/tutorial.html#ECharts%20%E4%B8%AD%E7%9A%84%E4%BA%8B%E4%BB%B6%E5%92%8C%E8%A1%8C%E4%B8%BA
   *
   * */
  ...
  export default {
    ...
    props: {
      ...
      showLabel: {
        type: Boolean,
        default: option.custom.basicSerie.label.normal.show || true
      },
      rendererType: {
        type: String,
        default: 'canvas'
      ...
      labelPosition: {
        type: String,
        default: option.custom.basicSerie.label.normal.position || 'outside'
      },
      showLegend: {
        type: Boolean,
        default: true
    ...
    data: function(){
      return {
        ...
        option: option,
        chart: null,//每个组件都需要，在initialize.js中会用到
    ...
    methods: {
      ...
      setDataInfo: function(obj){
        ...
        let _this = this,
          id = _this.id,
          seriesArr = [],
          basicSerie = _this.option.custom.basicSerie,
          option = _this.tool.extend(true,{},obj),
        ...
        if(_this.hasChild){
          ...
          option.series[0].data = _this.data;
          option.series[0].radius = _this.radius;
          option.series[0].center = _this.center;
          option.series[0].label.normal.position = labelPosition;
          if(_this.rippleAnimation){
            let ripple = _this.tool.extend(true,{},_this.rippleOption);
            ripple.data[0] = _this.computeCoordinate(_this.center,id);
